...
resource "google_container_node_pool" "actions-runner-pool" {
  ...
  node_count = var.min_main_node_count 
  management {
    auto_repair  = "true"
    auto_upgrade = "true"
   }
...
resource "google_container_node_pool" "actions-runner-pool-memory" {
  name       = "${var.environment}-memory-pool"
  cluster    = google_container_cluster.actions-runner-gke.name
  location   = google_container_cluster.actions-runner-gke.location
  
  autoscaling {
    min_node_count = var.min_memory_node_count
    max_node_count = var.max_memory_node_count
   }
  node_count = var.min_memory_node_count
  management {
    auto_repair  = "true"
    auto_upgrade = "true"
   }
